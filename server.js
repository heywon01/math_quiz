...
app.post('/api/problems', async (req, res) => {
    ...
    try {
        ...
        if (err.code === 11000) return res.status(409).send('이미 해당 날짜의 문제가 존재합니다.');
        res.status(500).send(err.message);
    }
});
...
app.delete('/api/problems/:date', async (req, res) => {
    ...
    try {
        ...
        if (result.deletedCount === 0) {
            return res.status(404).send('해당 날짜의 문제를 찾을 수 없습니다.');
        }
...
app.post('/api/problems/:date/solve', async (req, res) => {
    ...
    try {
        ...
        if (!problem || !user) return res.status(404).send('문제 또는 사용자를 찾을 수 없습니다.');
        
        // 이미 푼 사용자인지 확인
        const alreadySolved = problem.solvers.some(s => s.userId.equals(user._id));
        if (alreadySolved) return res.status(400).send('이미 이 문제를 풀었습니다.');
